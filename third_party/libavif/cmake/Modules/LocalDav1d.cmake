...
set(AVIF_DAV1D_ENABLE_ASM
    "AUTO"
    CACHE STRING "Enable dav1d assembly optimizations. AUTO enables them only when NASM or YASM is available."
)
set_property(CACHE AVIF_DAV1D_ENABLE_ASM PROPERTY STRINGS AUTO ON OFF)
...
function(avif_build_local_dav1d)
    ...
    # RowPack compatibility patch: dav1d's x86 assembly is fast, but requiring
    # NASM makes a first Windows build fail. Keep assembly on when the assembler
    # exists, and otherwise build dav1d's portable C path.
    set(DAV1D_ENABLE_ASM_VALUE true)
    if(AVIF_DAV1D_ENABLE_ASM STREQUAL "ON")
        set(DAV1D_ENABLE_ASM_VALUE true)
    elseif(AVIF_DAV1D_ENABLE_ASM STREQUAL "OFF")
        set(DAV1D_ENABLE_ASM_VALUE false)
    else()
        find_program(DAV1D_NASM_EXECUTABLE NAMES nasm)
        find_program(DAV1D_YASM_EXECUTABLE NAMES yasm)
        if(DAV1D_NASM_EXECUTABLE OR DAV1D_YASM_EXECUTABLE)
            set(DAV1D_ENABLE_ASM_VALUE true)
        else()
            set(DAV1D_ENABLE_ASM_VALUE false)
            message(STATUS "libavif(AVIF_CODEC_DAV1D=LOCAL): NASM/YASM not found; configuring dav1d with -Denable_asm=false")
        endif()
    endif()
    ...
    set(DAV1D_MESON_SETUP_ARGS
        setup
        --buildtype=release
        --default-library=static
        --prefix=<INSTALL_DIR>
        --libdir=lib
        -Denable_asm=${DAV1D_ENABLE_ASM_VALUE}
        -Denable_tools=false
    ...
    ExternalProject_Add(
        ...
        UPDATE_COMMAND ""
        CONFIGURE_COMMAND
            ${CMAKE_COMMAND} -E env "PATH=${PATH}" ${MESON_EXECUTABLE} ${DAV1D_MESON_SETUP_ARGS} <SOURCE_DIR>
